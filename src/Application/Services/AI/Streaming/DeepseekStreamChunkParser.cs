...
public class DeepseekStreamChunkParser : BaseStreamChunkParser<DeepseekStreamChunkParser>
{
    ...
    private void ParseFunctionCallObject(ref Utf8JsonReader reader, ref ToolCallChunk? toolCallInfo)
    {
        ...
        while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
        {
            ...
            switch (propertyName)
            {
                case "name":
                    ...
                    break;
                case "arguments":
                    functionArguments = reader.GetString();
                    break;
                default:
                    reader.Skip();
                    break;
        ...
        {
            toolCallInfo = new ToolCallChunk(0, null, functionName, functionArguments);
        }
    }
        ...
        if (root.TryGetProperty("choices", out var choicesElement) && choicesElement.ValueKind == JsonValueKind.Array)
        {
            foreach (var choice in choicesElement.EnumerateArray())
            {
                        ...
                        foreach (var toolCall in toolCallsElement.EnumerateArray())
                        {
                            if (toolCall.TryGetProperty("function", out var functionElement))
                            {
                                if (functionElement.TryGetProperty("name", out var nameElement) && nameElement.ValueKind == JsonValueKind.String)
                                {
                                    ...
                                    Logger?.LogTrace("Parsed DeepSeek function call: {FunctionName}", functionName);
                                }
                            }
                        }
                    }
                ...
                if (choice.TryGetProperty("finish_reason", out var finishReasonElement) && finishReasonElement.ValueKind == JsonValueKind.String)
                {
                    ...
                    Logger?.LogDebug("Parsed DeepSeek finish reason: {FinishReason}", finishReason);
                }
            }
        }
